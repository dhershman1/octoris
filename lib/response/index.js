...
const logger = require('pino')({ name: 'octoris/response' })
...
function json (code, data) {
  ...
  if (and(codeType !== 'Number', codeType !== 'String')) {
    logger.info('Code was incorrect type throwing error')
  ...
  return function ({ response }) {
    ...
    logger.info('Responding with %o', data)
...
function redirect (url, data) {
  return function ({ response, request }) {
    ...
    if (request.method === 'HEAD') {
      logger.info('HEAD method used')
      return response.end()
    }

    logger.info('Redirecting with %o', data)
