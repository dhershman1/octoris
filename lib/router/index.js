...
const octo = logger({ name: 'octoris/router' })
const { first, last, omit, reduce, type, update } = require('kyanite')

const { tree, searchTree } = require('./tree')
const { toPairs, flattenMap } = require('../utils')
...
function route (paths, methods) {
  if (methods) {
    ...
    octo.info('Finished Branch: %o', toPairs(res))
    return res
  }
...
function composeRoutes (opts, routes) {
  ...
  const extra = opts.logger ? { logger: useLogger(opts.logger) } : {}
  ...
  return function (request, response) {
    ...
    if (foundMap) {
      octo.info('Found %O', toPairs(foundMap))
      ...
      octo.info('Context', omit(['request', 'response'], ctx))
      octo.info('Found Listener %o', method)
