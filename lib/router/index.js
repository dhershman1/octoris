...
function composeRoutes (opts, routes, mw = []) {
  ...
  return function (request, response) {
    ...
    return runMiddleware(ctx, globalMiddleware)
      .then(globalCtx => {
        if (globalCtx.node.response.finished) {
          return
        }
        ...
        if (foundMap) {
          ...
          return runMiddleware(globalCtx, routeMiddleware)
            .then(routeCtx => runMiddleware(routeCtx, middleware))
...
module.exports = {
  composeRoutes: _curry3(composeRoutes),
