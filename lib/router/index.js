...
const { first, last, reduce, type, update, pipeP, or } = require('kyanite')
...
function composeRoutes (opts, routes, mw = []) {
  ...
  return function (request, response) {
    ...
    return pipeP(globalMiddleware, ctx)
      .then(globalCtx => {
        if (or(globalCtx.node.response.finished, globalCtx.response.finished)) {
          return
        }
        ...
        if (foundMap) {
          ...
          return pipeP(routeMiddleware, globalCtx)
