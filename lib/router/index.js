...
function route (paths, methods) {
  if (methods) {
    ...
    logger.info('Finished Branch: %o', toPairs(res))
    return res
  }
...
function routeReducer (routes) {
  ...
  return function (request, response) {
    ...
    if (foundMap) {
      logger.info('Found %O', toPairs(foundMap))
      ...
      logger.info('Context', omit(['request', 'response'], ctx))
      logger.info('Found Listener %o', method)
